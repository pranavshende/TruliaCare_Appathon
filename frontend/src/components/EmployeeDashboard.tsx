  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold">{user?.role === 'TECHNICIAN' ? 'My Assigned Tickets' : 'My Requests'}</h2>
              ...
              <tr key={r.id}>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                  <div className="flex space-x-2">
                    ...
                    {user?.role === 'TECHNICIAN' && r.status === 'PENDING' && (
                      <button 
                        onClick={async () => {
                          const res = await apiFetch(`/requests/${r.id}/accept`, { method: 'PATCH' });
                    ...
                    {user?.role === 'TECHNICIAN' && r.status === 'IN_PROGRESS' && (
                      <button 
